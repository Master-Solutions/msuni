...
export const RoutingAspect = <
	T extends AnyConstructor<
		Context & ResourceManagementAspect & ComponentsRegistryAspect & ReactContextProvidersAspect & PagesAspect
	>
>(
	base: T
) => {
	class Routing extends base {
		...
		createRoute(props: RouteProps) {
			...
			const Page = this.getPageComponent(props.page);

			const routeComponentKey = type ? `routing.${type}_route` : 'routing.route';
			const Route = this.getComponent(routeComponentKey);
			if (!Route) throw new Error(`Component '${routeComponentKey}' for route type '${type}' is not registered`);
			...
			return React.createElement(Route, routeProps, React.createElement(Page));
		}
